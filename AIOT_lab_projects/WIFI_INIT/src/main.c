...
int main(void)
{
  ...
  char message[50];
  if (WIFI_JoinNetwork(&hwifi) == WIFI_STATUS_OK) {
      sprintf(message, "Joined WiFi Network: %s\r\n", ssid);
      HAL_UART_Transmit(&huart1, (uint8_t *)message, strlen(message), HAL_MAX_DELAY);
  } else {
      sprintf(message, "Failed to Join WiFi Network: %s\r\n", ssid);
      HAL_UART_Transmit(&huart1, (uint8_t *)message, strlen(message), HAL_MAX_DELAY);
  }
  /* USER CODE END 2 */
...
static void WIFI_Init_main(){
	...
	WIFI_Init(&hwifi);
	HAL_UART_Transmit(&huart1, (uint8_t *)"WiFi Initialized\r\n", strlen("WiFi Initialized\r\n"), HAL_MAX_DELAY);
