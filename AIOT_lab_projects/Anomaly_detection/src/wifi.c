...
WIFI_StatusTypeDef WIFI_Init(WIFI_HandleTypeDef* hwifi) {
    ...
    WIFI_RESET_MODULE();
    HAL_Delay(1000);  // Allow time for the module to reset
    ...
    WIFI_DISABLE_NSS();
    HAL_Delay(500); // Allow some time after disabling NSS

    // Send initial configuration commands
    msgLength = sprintf(wifiTxBuffer, "Z3=0\r");
    if (WIFI_SendATCommand(hwifi, wifiTxBuffer, WIFI_TX_BUFFER_SIZE, wifiRxBuffer, WIFI_RX_BUFFER_SIZE) != WIFI_OK) {
        Error_Handler();
    }

    if (strstr(wifiRxBuffer, "OK") == NULL) {
        Error_Handler();
    }
...
WIFI_StatusTypeDef WIFI_JoinNetwork(WIFI_HandleTypeDef* hwifi) {
    ...
    if (WIFI_SendATCommand(hwifi, wifiTxBuffer, msgLength + 1, wifiRxBuffer, WIFI_RX_BUFFER_SIZE) != WIFI_OK) {
        return WIFI_ERROR;
    }
...
WIFI_StatusTypeDef WIFI_MQTTClientInit(WIFI_HandleTypeDef* hwifi) {
    ...
    // Set publish topic
    printf("Initializing MQTT Client with Publish Topic: %s\n", hwifi->mqtt.publishTopic);
    msgLength = sprintf(wifiTxBuffer, "PM=0,%s\r", hwifi->mqtt.publishTopic);
    if (WIFI_SendATCommand(hwifi, wifiTxBuffer, msgLength + 1, wifiRxBuffer, WIFI_RX_BUFFER_SIZE) != WIFI_OK) {
        return WIFI_ERROR;
    }

    // Set subscribe topic only if it's not empty
    if (strlen(hwifi->mqtt.subscribeTopic) > 0) {
        printf("Initializing MQTT Client with Subscribe Topic: %s\n", hwifi->mqtt.subscribeTopic);
